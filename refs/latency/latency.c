...
char*             pdevice        = "hw:PCH,0";
char*             cdevice        = "hw:PCH,0";
...
static inline long long frames_to_micro(size_t frames) { return (long long)((frames * 1000000LL) + (rate / 2)) / rate; }
                      ...
                      const char*          id)
{
    ...
    if (period_size > 0) periodsize = period_size;
    else periodsize /= 2;
...
int setparams_set(snd_pcm_t* handle, snd_pcm_hw_params_t* params, snd_pcm_sw_params_t* swparams, const char* id)
{
    ...
    if (!block) val = 4;
    else snd_pcm_hw_params_get_period_size(params, &val, NULL);
...
__again:
    ...
    if (last_bufsize == *bufsize) *bufsize += 4;
    last_bufsize = *bufsize;
    if (*bufsize > latency_max) return -1;
__set_it:
    ...
    if (p_psize > (unsigned int)*bufsize) *bufsize = p_psize;
    snd_pcm_hw_params_get_period_size(c_params, &c_psize, NULL);
    if (c_psize > (unsigned int)*bufsize) *bufsize = c_psize;
    snd_pcm_hw_params_get_period_time(p_params, &p_time, NULL);
    snd_pcm_hw_params_get_period_time(c_params, &c_time, NULL);
    if (p_time != c_time) goto __again;
...
long readbuf(snd_pcm_t* handle, char* buf, long len, size_t* frames, size_t* max)
{
    ...
    if (!block)
    {
        do { r = snd_pcm_readi(handle, buf, len); } while (r == -EAGAIN);
        if (r > 0)
        {
            *frames += r;
            if ((long)*max < r) *max = r;
...
{
    int k;
    printf("Usage: latency [OPTION]... [FILE]...\n"
           "-h,--help      help\n"
...
int main(int argc, char* argv[])
{
    struct option long_option[] = {
        {"help", 0, NULL, 'h'},     {"pdevice", 1, NULL, 'P'}, {"cdevice", 1, NULL, 'C'},  {"min", 1, NULL, 'm'},
        {"max", 1, NULL, 'M'},      {"updates", 1, NULL, 'U'}, {"frames", 1, NULL, 'F'},   {"format", 1, NULL, 'f'},
        {"channels", 1, NULL, 'c'}, {"rate", 1, NULL, 'r'},    {"buffer", 1, NULL, 'B'},   {"period", 1, NULL, 'E'},
        {"seconds", 1, NULL, 's'},  {"block", 0, NULL, 'b'},   {"poll", 0, NULL, 'p'},     {"usleep", 1, NULL, 'y'},
        {"effect", 0, NULL, 'e'},   {"posdump", 0, NULL, 'x'}, {"realtime", 0, NULL, 'X'}, {"policy", 1, NULL, 'O'},
        {NULL, 0, NULL, 0},
    };
    ...
    {
        int c;
        if ((c = getopt_long(argc, argv, "hP:C:m:M:U:F:f:c:r:B:E:s:y:O:bpenxX", long_option, NULL)) < 0) break;
        switch (c)
        {
        case 'h': morehelp++; break;
        case 'P': pdevice = strdup(optarg); break;
        case 'C': cdevice = strdup(optarg); break;
        case 'm':
            err         = atoi(optarg) / 2;
            latency_min = err >= 4 ? err : 4;
            if (latency_max < latency_min) latency_max = latency_min;
            break;
        case 'M':
            err         = atoi(optarg) / 2;
        ...
        case 'p': use_poll = 1; break;
        case 'y': usleep_val = atoi(optarg); break;
        case 'e': effect = 1; break;
        case 'n': resample = 0; break;
        case 'x': pos_dump = 1; break;
        case 'X': realtime_check = 1; break;
        case 'O': sched_policy = optarg; break;
        }
    }
    ...
    printf("Parameters are %iHz, %s, %i channels, %s mode, use poll %s\n", rate, snd_pcm_format_name(format), channels,
           block ? "blocking" : "non-blocking", use_poll ? "yes" : "no");
    printf("Loop limit is %lu frames, minimum latency = %i, maximum latency = %i", loop_limit, latency_min * 2,
           latency_max * 2);
    if (sys_latency > 0) printf(", I/O updates %ims", sys_latency);
    ...
    while (1)
    {
        ...
        if (setparams(phandle, chandle, &latency) < 0) break;
        showlatency(latency);
        ...
        if (realtime_check) timestamp_now(&tstamp_start);
        if ((err = snd_pcm_start(chandle)) < 0)
        {
            printf("Go error: %s\n", snd_strerror(err));
            exit(0);
        }
        if (realtime_check) printf("[%lldus] Stream start\n", timestamp_diff_micro(&tstamp_start));
        ...
        while (ok && frames_in < loop_limit)
        {
            ...
            else if (!block && usleep_val > 0) { usleep(usleep_val); }
            if (pos_dump || realtime_check)
            {
                if (sys_latency <= 0) cap_avail = get_avail(chandle);
                pbk_fill = get_avail(phandle);
                if (pbk_fill >= 0) pbk_fill = playback_buffer_size - pbk_fill;
                if (cap_avail > cap_avail_max) cap_avail_max = cap_avail;
                if (pbk_fill >= 0 && pbk_fill < pbk_fill_min) pbk_fill_min = pbk_fill;
                if (realtime_check)
                {
                    ...
                    printf("[%lldus] POS: p=%zd (min=%zd, rt=%lldus) c=%zd (max=%zd, rt=%lldus)\n", diff, pbk_fill,
                           pbk_fill_min, pbk_pos - diff, cap_avail, cap_avail_max, cap_pos - diff);
                }
                else if (pos_dump)
                {
        ...
        showstat(chandle, frames_in);
        showinmax(in_max);
        if (p_tstamp.tv_sec == c_tstamp.tv_sec && p_tstamp.tv_usec == c_tstamp.tv_usec) printf("Hardware sync\n");
        if (realtime_check)
        {
            long long diff  = timestamp_diff_micro(&tstamp_start);
        ...
        if (ok)
        {
            ...
            printf("Playback time = %li.%i, Record time = %li.%i, diff = %li\n", (long)p_tstamp.tv_sec,
                   (int)p_tstamp.tv_usec, (long)c_tstamp.tv_sec, (int)c_tstamp.tv_usec, timediff(p_tstamp, c_tstamp));
#endif
            break;
        }
