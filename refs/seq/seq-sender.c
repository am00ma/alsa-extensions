...
void set_hwparams(snd_pcm_t* phandle)
{
    ...
    err = snd_pcm_hw_params_set_access(phandle, params, SND_PCM_ACCESS_RW_INTERLEAVED);
    if (err < 0)
    {
...
void send_event(snd_seq_t* handle, int queue, int client, int port, snd_seq_addr_t* dest, int* time)
{
    int             err;
    snd_seq_event_t ev;
    ...
    if ((err = snd_seq_event_output(handle, &ev)) < 0) fprintf(stderr, "Event output error: %s\n", snd_strerror(err));
    if ((err = snd_seq_drain_output(handle)) < 0) fprintf(stderr, "Event drain error: %s\n", snd_strerror(err));
}
...
#ifdef USE_PCM
    if (phandle) max += snd_pcm_poll_descriptors_count(phandle);
#endif
    pfds = alloca(sizeof(*pfds) * max);
    while (1)
    {
        int nseqs = snd_seq_poll_descriptors_count(handle, POLLOUT | POLLIN);
        if (snd_seq_event_output_pending(handle)) snd_seq_poll_descriptors(handle, pfds, nseqs, POLLOUT | POLLIN);
        else snd_seq_poll_descriptors(handle, pfds, nseqs, POLLIN);
        max = nseqs;
#ifdef USE_PCM
        if (phandle)
        {
...
#endif
        if (pfds[0].revents & POLLOUT) snd_seq_drain_output(handle);
        if (pfds[0].revents & POLLIN)
        {
            do
            {
                ...
                if (!ev) continue;
                if (ev->type == SND_SEQ_EVENT_ECHO) send_event(handle, queue, client, port, &addr, &time);
                decode_event(ev);
